$(document).ready(function () {
    if (typeof (admin_id) != 'undefined')
        var the_url = baseURL + "/admins/" + admin_id + "/logs-data";
    else
        var the_url = baseURL + "/logs/log-data";


    if ($("#log_tbl").length) {

        var log_tbl = $("#log_tbl");
        log_tbl.on('preXhr.dt', function (e, logs, data) {
            data.permission_id = $('#permission_id').val();
            data.admin_id = $('#admin_id_filter').val();
            data.date = $('#date').val();
            ...
            fnDrawCallback: function () {
                //Initialize checkbos for enable/disable log
        ...
        $('#log_tbl').on('change', '.make-switch.active', function (event, state) {
            // ... skipped ...
            var log_id = $(this).data('id');

            $.ajax({
                url: baseURL + '/logs/' + log_id + '/active',
                type: 'PUT',
                dataType: 'json',
        ...
        // $('#log_tbl').on('change', '.make-switch.open', function (event, state) {
        $('#log_tbl').on('change', '.make-switch.open', function (event, state) {
            // ... skipped ...
            var log_id = $(this).data('id');

            $.ajax({
                url: baseURL + '/logs/' + log_id + '/open',
                type: 'PUT',
                dataType: 'json',
                data: {'_token': csrf_token},
                success: function (data) {
    ...
    $(document).on('click', '.delete', function (event) {
        ...
        var log_name = _this.closest('tr').find("td:eq(2)").text();
        bootbox.dialog({
            message: "Do you want for deleting (" + log_name + ")? <span class=' label-danger'> can't return back</span>",
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status) {
                    ...
                    if (log_tbl != null && log_tbl != undefined)
                        log_tbl.api().ajax.reload();
