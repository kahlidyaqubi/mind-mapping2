$(document).ready(function () {
    var the_url = baseURL + "/notifications/notification-data";

    if ($("#notification_tbl").length) {

        var notification_tbl = $("#notification_tbl");
        notification_tbl.on('preXhr.dt', function (e, notifications, data) {
            data.date = $('#date').val();
            data.read_at = $('#read_at').val();
        }).dataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            ...
            fnDrawCallback: function () {
                //Initialize checkbos for enable/disable notification
        ...
        $('#notification_tbl').on('change', '.make-switch.active', function (event, state) {
            // ... skipped ...
            var notification_id = $(this).data('id');

            $.ajax({
                url: baseURL + '/notifications/' + notification_id + '/active',
                type: 'PUT',
                dataType: 'json',
        ...
        // $('#notification_tbl').on('change', '.make-switch.open', function (event, state) {
        $('#notification_tbl').on('change', '.make-switch.open', function (event, state) {
            // ... skipped ...
            var notification_id = $(this).data('id');

            $.ajax({
                url: baseURL + '/notifications/' + notification_id + '/open',
                type: 'PUT',
                dataType: 'json',
                data: {'_token': csrf_token},
                success: function (data) {
    ...
    $(document).on('click', '.filter-cancel', function () {
        ...
        // $('#is_notification_confirm,.status').val('').trigger('change');
        notification_tbl.api().ajax.reload();
    });
    ...
    $(document).on('click', '.delete', function (event) {
        ...
        var notification_name = _this.closest('tr').find("td:eq(2)").text();
        bootbox.dianotification({
            message: "Do you want for deleting (" + notification_name + ")? <span class=' label-danger'> can't return back</span>",
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status) {
                    ...
                    if (notification_tbl != null && notification_tbl != undefined)
                        notification_tbl.api().ajax.reload();
